...
3. Open the extension options. Optional: a notify adapter (Hermes local webhook, or a Telegram bot) for login-lost alerts, and an ingest URL for ExpiredDomains.
...
Stay time and caps are in the page panel. Ingest POSTs only to `127.0.0.1` or `::1`. Alerts go through the selected notify adapter (Hermes webhook or Telegram bot). One job at a time: the extension opens its own tab, then closes it.
