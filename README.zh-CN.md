...
3. 把 `config.example.yaml` 复制成 `extension/config.yaml`，在里面填通知/ingest。改完重新加载扩展。
...
停留和上限在页内面板改。ingest 只发到 `127.0.0.1` / `::1`。提醒走 `config.yaml` 里选的通知适配器（Hermes webhook 或 Telegram bot）。同一时刻只跑一个任务：扩展自己开标签，做完关掉。
